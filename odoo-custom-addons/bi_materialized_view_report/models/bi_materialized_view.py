        ...
        try:
            self.env.cr.execute("""
                ...
                SELECT
                    ...
                    -- ✅ NEW METRICS:
                    COUNT(DISTINCT sr.order_id) AS x_total_orders,
                    COUNT(DISTINCT sr.product_id) AS x_total_sku,
                    CASE
                        WHEN COUNT(DISTINCT sr.order_id) > 0 THEN 
                            ROUND(SUM(sr.product_uom_qty)::numeric / COUNT(DISTINCT sr.order_id), 2)
                        ELSE 0
                    END AS x_basket_size
                ...
                LEFT JOIN product_supplierinfo psi ON pt.id = psi.product_tmpl_id AND psi.sequence = 1
