...
class wizard_inventory_qty(models.TransientModel):
    _name = 'wizard.inventory.qty'
    ...
    def print_report(self):
        ...
        return self.env.ref('eq_inventory_qty_report.action_inventory_qty_template').report_action(self,
                                                                                                               data=datas)
            ...
            if not self.location_ids:
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        ending_qty_val = (
                                beginning_qty_val + product_qty_in_val + product_qty_out_val + product_qty_internal_val + product_qty_adjustment_val)
                    ...
                    for categ, product_value in product_val.items():
                        ...
                        for product in product_value:
                            ...
                            worksheet.write(rows, 8, formatLang(self.env, abs(product_qty_out_val)), header_data_format)
                            worksheet.write(rows, 9, product_qty_internal, header_data_format)
                            worksheet.write(rows, 10, formatLang(self.env, product_qty_internal_val),
                                            header_data_format)
                            worksheet.write(rows, 11, product_qty_adjustment, header_data_format)
                            worksheet.write(rows, 12, formatLang(self.env, product_qty_adjustment_val),
                                            header_data_format)
                            worksheet.write(rows, 13, ending_qty, header_data_format)
                            worksheet.write(rows, 14, formatLang(self.env, ending_qty_val), header_data_format)
                        ...
                        worksheet.write(rows, 4, formatLang(self.env, categ_prod_beginning_qty_val),
                                        header_merge_format)
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        for location, value in location_wise_data[0].items():
                            ...
                            worksheet.write(rows, 5, formatLang(self.env,
                                                                report_stock_inv_obj.get_product_valuation(self,
                                                                                                           product,
                                                                                                           value[0],
                                                                                                           warehouse,
                                                                                                           'beg')),
                            ...
                            worksheet.write(rows, 7, formatLang(self.env,
                                                                report_stock_inv_obj.get_product_valuation(self,
                                                                                                           product,
                                                                                                           value[1],
                                                                                                           warehouse,
                            ...
                            worksheet.write(rows, 9, formatLang(self.env,
                                                                abs(report_stock_inv_obj.get_product_valuation(self,
                                                                                                               product,
                                                                                                               abs(
                                                                                                                   value[
                                                                                                                       2]),
                            ...
                            worksheet.write(rows, 11, formatLang(self.env,
                                                                 report_stock_inv_obj.get_product_valuation(self,
                                                                                                            product,
                                                                                                            value[3],
                                                                                                            warehouse,
                            ...
                            worksheet.write(rows, 13, formatLang(self.env,
                                                                 report_stock_inv_obj.get_product_valuation(self,
                                                                                                            product,
                                                                                                            value[4],
                                                                                                            warehouse,
                            ...
                            worksheet.write(rows, 15, formatLang(self.env,
                                                                 report_stock_inv_obj.get_product_valuation(self,
                                                                                                            product,
                                                                                                            value[5],
                                                                                                            warehouse,
                    ...
                    for categ, product_value in product_val.items():
                        ...
                        for product in product_value:
                            ...
                            product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self, product_id,
                                                                                                    ...
                                                                                                    warehouse, 'adj')
                            ending_qty = location_wise_data[1][5]
                            ending_qty_val = report_stock_inv_obj.get_product_valuation(self, product_id, ending_qty,
                                                                                        warehouse, 'final')
                        ...
                        worksheet.write(rows, 5, formatLang(self.env, categ_prod_beginning_qty_val),
                                        header_merge_format)
						...
						# Read the generated Excel file and encode it
						with open(filepath, 'rb') as file:
							xls_data = base64.b64encode(file.read())

						# Delete the temporary Excel file
						os.remove(filepath)

						# Update the Odoo record with the Excel file data
						self.write({
							'state': 'get',
							'data': xls_data,
							'name': xls_filename
						...
						# Return an action to open the Excel file for printing
						return {
							'type': 'ir_actions_report',
							'report_type': 'xlsx',
							'report_name': 'inventory_valuation_report.xlsx',
							'name': 'Inventory Valuation Report',
							'datas': {'model': self._name, 'ids': [self.id], 'filename': xls_filename},
