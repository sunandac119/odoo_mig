...
class eq_inventory_qty_report_inventory_qty_report(models.AbstractModel):
    _name = 'report.eq_inventory_qty_report.inventory_qty_report'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']._get_report_from_name('eq_inventory_qty_report.inventory_qty_report')
        record_id = data['form']['id'] if data and data['form'] and data['form']['id'] else docids[0]
        records = self.env['wizard.inventory.qty'].browse(record_id)
        return {
