...
class PosBrand3DayPagesWizard(models.TransientModel):
    _name = "pos.brand.3day.pages.wizard"
    ...
    def _collect(self):
        ...
        Returns:
        brands = [
          {'brand': 'Brand A', 'total_qty': 123.0, 'products': [
              {'name': 'Prod 1', 'barcode': '...', 'uom': 'Unit', 'qty': 10.0},
              ...
          ]},
          ...
        ]
        Sorted by brand name A→Z; products by qty DESC.
        """
        ...
        for row in rg:
            ...
            # store meta once (first encountered variant becomes barcode source)
            if tmpl.id not in tmpl_meta:
                # Prefer the exact product’s barcode if first time; else any variant
    ...
    def action_print_pdf(self):
        ...
        return self.env.ref('pos_brand_3day_by_brand_pages.action_report_pos_brand_3day_pages').report_action(self, data=data)
