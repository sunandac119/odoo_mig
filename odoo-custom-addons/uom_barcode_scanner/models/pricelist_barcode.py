...
class ProductPricelistItem(models.Model):
    ...
    # @api.onchange('uom_id')
    # def onchange_uom(self):
    #     _logger.info("=== Onchange triggered for UoM ===")
    #     _logger.info("Product ID: %s | Current UoM ID: %s", self.product_id.id if self.product_id else 'N/A', self.uom_id.id if self.uom_id else 'N/A')

    #     self.applied_on = '1_product'
    #     _logger.info("Applied_on set to '1_product'")

    #     barcode_line = self.env['product.barcode.uom'].search([
    #         ('product_id', '=', self.product_id.product_tmpl_id.id if self.product_id else False),
    #         ('uom_id', '=', self.uom_id.id if self.uom_id else False)
    #     ], limit=1)

    #     _logger.info(
    #         "Barcode search result — Found: %s | Product Template ID: %s | UoM ID: %s",
    #         bool(barcode_line),
    #         self.product_id.product_tmpl_id.id if self.product_id else 'N/A',
    #         self.uom_id.id if self.uom_id else 'N/A'
    #     )

    #     if barcode_line:
    #         self.x_scanned_barcode = barcode_line.barcode
    #         _logger.info("Matched barcode line: %s | Barcode: %s", barcode_line.id, barcode_line.barcode)
    #     else:
    #         _logger.warning(
    #             "No matching barcode found for Product Template ID: %s and UoM ID: %s",
    #             self.product_id.product_tmpl_id.id if self.product_id else 'N/A',
    #             self.uom_id.id if self.uom_id else 'N/A'
    #         )
            ...
            for rule in items:
                ...
                if price is not False:
                    barcode_line = self.env['product.barcode.uom'].search([
                        ...
                        ('product_id', '=', product.id if product._name == 'product.template' else product.product_tmpl_id.id),
                        ('uom_id', '=', price_uom.id)
                    ], limit=1)
                    if barcode_line and barcode_line.sale_price:
