...
class ResPartner(models.Model):
    ...
    is_vendor_or_customer = fields.Selection(selection=[('customer', 'Customer'), ('vendor', 'Vendor')],
                                             string="Are you an Vendor or Customer?")
    ...
    def peppol_kyc_needs_complete(self):
        if not self.peppol_sml_pdf_signed:
            raise UserError(_("Needs to add ans Peppol SML PDF Signed"))
        lhdn_setup_id = self.env['lhdn.setup'].search([], limit=1)
        peppol_participants_data_dict = {
            'api_client_id': lhdn_setup_id.peppol_sync_api_client_id,
            'api_client_password': lhdn_setup_id.peppol_sync_api_client_password,
            'peppol_participants_kyc_pdf': self.peppol_sml_pdf_signed,
            'participants_peppol_id': self.peppol_id
        }

        headers = {'Content-Type': 'application/json'}
        res = requests.post(f"{lhdn_setup_id.peppol_sync_api_base_url}/v1/peppol/kyc/peppol_participants",
                            json=peppol_participants_data_dict,
                            headers=headers)
