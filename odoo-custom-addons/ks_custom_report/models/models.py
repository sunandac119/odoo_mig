...
class KsCustomReport(models.Model):
    ...
    def _create_chain_query(self, rec):
        ...
        for column_id in rec.ks_cr_column_ids:
            ...
            if query_data.get(current_abbr_chain):
                ...
                if (field_id.ttype == 'datetime' or field_id.ttype == 'date') and any([ks_year, ks_month, ks_week, ks_day, ks_hour]):
                    field = "coalesce(%s, (now() at time zone 'UTC'))" % (current_abbr_chain + '.' + json.dumps(field)) + ' + interval ' + "'" + ' ' + ks_year + ' ' + ks_month + ' ' + ks_week + ' ' + ks_day + ' ' + ks_hour + "'"
                    ...
                    select_clause_list.append(current_abbr_chain + '.' + json.dumps(field) + ' as ' + json.dumps(ks_field_name))
        ...
        for field in header_rec:
            if field[0] != 'x_name' and rec.ks_cr_model_id and field[0] != 'id':
                field_id = self.env['ir.model.fields'].create({
                    ...
                    'model_id': rec.ks_cr_model_id.id,
                    'state': 'manual',
                    'ttype': self.ks_get_type_of_field(field),
                })
    ...
    def _ks_create_db_table(self, rec):
        ...
        if rec.ks_cr_query_type == 'field_chain':
            ...
            self.ks_create_tree_view(rec, table_query[2])
            self.ks_create_search_filter_view(rec, table_query[3], table_query[4])
        else:
            ...
            if 'union all' in ks_query or 'union' in ks_query:
                    ...
                    if 'select' in query:
                        ...
                        if 'row_number' not in query and 'as id' not in query:
                            if 'x_name' not in query:
                                query = query[:index_value] + "row_number() over() as id, '' as x_name," + query[index_value:]
                                ks_name_flag = True
                                ks_id_flag = True
                            else:
                ...
                if 'row_number' not in ks_query and 'as id' not in ks_query:
                    if 'x_name' not in ks_query:
                        ks_query = ks_query[:index_value] + "row_number() over() as id, '' as x_name," + ks_query[index_value:]
                        ks_name_flag = True
                        ks_id_flag = True
                    else:
    ...
    def ks_create_search_filter_view(self, rec, search_filter_field_list, group_filter_field_list):
        ...
        values = self.ks_group_filter_arch(fields_desc, search_filter_field_list, group_filter_field_list, rec.name, cr_model.model)
