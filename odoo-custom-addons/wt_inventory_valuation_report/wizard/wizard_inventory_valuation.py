...
class wizard_inventory_valuation(models.TransientModel):
    ...
    def get_default_warehouse(self):
        return self.env['stock.warehouse'].search([('company_id', 'in', self.env.user.company_ids.ids)])

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.user.company_id.id)
    warehouse_ids = fields.Many2many('stock.warehouse', string='Warehouse',
        default=lambda self: self.get_default_warehouse().ids)
    ...
    new_format = fields.Boolean(default=True)
    suggestion = fields.Integer(string='Suggestion (Days)', default=37)

    @api.model
    def default_get(self, fields_list):
        res = super(wizard_inventory_valuation, self).default_get(fields_list)
        if not res.get('warehouse_ids'):
            res['warehouse_ids'] = self.get_default_warehouse().ids
        return res
    ...
    def onchange_seller_ids(self):
        ...
        if self.seller_ids:
            ...
            selected_suppliers = self.env['product.supplierinfo'].search([('name', 'in', self.seller_ids.ids)])
            product_ids = product_ids.filtered(lambda p: p.product_tmpl_id in selected_suppliers.mapped('product_tmpl_id'))
    ...
    def print_report(self):
        ...
        # Select all warehouses if none are selected
        if not self.warehouse_ids:
            self.warehouse_ids = self.get_default_warehouse()
        ...
        return self.env.ref('wt_inventory_valuation_report.action_inventory_valuation_template').report_action(self, data=datas)
    ...
    def print_xls_report(self):
        if self.new_format:
            raise ValidationError('New format is not available for xlsx report')
        self.check_date_range()
        ...
        report_stock_inv_obj = self.env['report.wt_inventory_valuation_report.inventory_valuation_report']

        header_merge_format = workbook.add_format({'bold':True, 'align':'center', 'valign':'vcenter', 'font_size':10, 'bg_color':'#D3D3D3', 'border':1})
            ...
            if not self.location_ids:
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        beginning_qty_val = report_stock_inv_obj.get_product_valuation(self,product,beginning_qty,warehouse,'beg')
                        ...
                        product_qty_in_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_in,warehouse,'in')
                        product_qty_out = product_val.get('product_qty_out')
                        product_qty_out_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_out,warehouse,'out')
                        product_qty_internal = product_val.get('product_qty_internal')
                        product_qty_internal_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_internal,warehouse,'int')
                        product_qty_adjustment = product_val.get('product_qty_adjustment')
                        product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_adjustment,warehouse,'adj')

                        ending_qty = beginning_qty + product_qty_in + product_qty_out\
                                    + product_qty_internal + product_qty_adjustment
                        
                        ending_qty_val = (beginning_qty_val + product_qty_in_val + product_qty_out_val + product_qty_internal_val + product_qty_adjustment_val)
                    ...
                    for categ, product_value in product_val.items():
                        ...
                        worksheet.merge_range(rows, 0, rows, 14, self.env['product.category'].browse(categ).name, header_merge_format)
                        rows += 1
                        for product in product_value:
                            ...
                            beginning_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,beginning_qty,warehouse,'beg')
                            product_qty_in = product.get('product_qty_in')
                            product_qty_in_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_in,warehouse,'in')
                            product_qty_out = product.get('product_qty_out')
                            product_qty_out_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_out,warehouse,'out')
                            product_qty_internal = product.get('product_qty_internal')
                            product_qty_internal_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_internal,warehouse,'int')
                            product_qty_adjustment = product.get('product_qty_adjustment')
                            product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_adjustment,warehouse,'adj')
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product,ending_qty,warehouse,'final')
                        ...
                        for location, value in location_wise_data[0].items():
                            ...
                            worksheet.write(rows, 5, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[0],warehouse,'beg')), header_data_format)
                            worksheet.write(rows, 6, value[1], header_data_format)
                            worksheet.write(rows, 7, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[1],warehouse,'in')), header_data_format)
                            worksheet.write(rows, 8, abs(value[2]), header_data_format)
                            worksheet.write(rows, 9, formatLang(self.env, abs(report_stock_inv_obj.get_product_valuation(self,product,abs(value[2]),warehouse,'out'))), header_data_format)
                            worksheet.write(rows, 10, value[3], header_data_format)
                            worksheet.write(rows, 11, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[3],warehouse,'int')), header_data_format)
                            worksheet.write(rows, 12, value[4], header_data_format)
                            worksheet.write(rows, 13, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[4],warehouse,'adj')), header_data_format)
                            worksheet.write(rows, 14, value[5], header_data_format)
                            worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[5],warehouse,'final')), header_data_format)
                            rows += 1
                    ...
                    for categ, product_value in product_val.items():
                        categ_prod_beginning_qty = categ_prod_qty_in = categ_prod_qty_out = categ_prod_qty_int = categ_prod_qty_adjust = categ_prod_ending_qty = 0.00
                        categ_prod_beginning_qty_val = categ_prod_qty_in_val = categ_prod_qty_out_val = categ_prod_qty_int_val = categ_prod_qty_adjust_val = categ_prod_ending_qty_val = 0.00
                        worksheet.merge_range(rows, 0, rows, 15, self.env['product.category'].browse(categ).name, header_merge_format)
                        rows += 1
                        for product in product_value:
                            product_id = self.env['product.product'].browse(product['product_id'])
                            ...
                            product_qty_adjustment_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_adjustment,warehouse,'adj')
                            ending_qty = location_wise_data[1][5]
                            ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,ending_qty,warehouse,'final')
                            ...
                            for location, value in location_wise_data[0].items():
                                ...
                                worksheet.write(rows, 3, location.display_name, header_data_format)
                                worksheet.write(rows, 4, value[0], header_data_format)
                                worksheet.write(rows, 5, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[0],warehouse,'beg')), header_data_format)
                                worksheet.write(rows, 6, value[1], header_data_format)
                                worksheet.write(rows, 7, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[1],warehouse,'in')), header_data_format)
                                worksheet.write(rows, 8, abs(value[2]), header_data_format)
                                worksheet.write(rows, 9, formatLang(self.env, abs(report_stock_inv_obj.get_product_valuation(self,product_id,abs(value[2]),warehouse,'out'))), header_data_format)
                                worksheet.write(rows, 10, value[3], header_data_format)
                                worksheet.write(rows, 11, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[3],warehouse,'int')), header_data_format)
                                worksheet.write(rows, 12, value[4], header_data_format)
                                worksheet.write(rows, 13, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[4],warehouse,'adj')), header_data_format)
                                worksheet.write(rows, 14, value[5], header_data_format)
                                worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[5],warehouse,'final')), header_data_format)
                                rows += 1
