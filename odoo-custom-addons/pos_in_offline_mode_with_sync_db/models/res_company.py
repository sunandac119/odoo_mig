...
FIELDS = {'account.tax.group': ['id', 'name'],
          ...
          'product.pricelist.item': ['id', 'name', 'applied_on', 'base', 'base_pricelist_id', 'categ_id',
                                     ...
                                     'price_round', 'pricelist_id', 'product_tmpl_id'],
         ...
         # 'user_ids',, 'title'
          'res.partner': ['id', 'name', 'date', 'parent_id', 'parent_name', 'child_ids', 'ref',
                          'vat', 'bank_ids',
                          'category_id', 'credit_limit', 'employee', 'type',
                          'email', 'mobile',
                          'barcode',
                          'email', 'phone', 'user_id',
                          'credit',
          ...
          'product.template': ['name', 'price', 'lst_price', 'default_code', 'type',
                               'barcode', 'description',
                               'available_in_pos', 'pos_categ_id', 'standard_price',
                               'cost_method', 'id', 'display_name','uom_id'],
...
class ResCompany(models.Model):
    ...
    current_running_db_server = fields.Selection(
        selection=[('local_server', 'Local Server'), ('remote_server', 'Remote Server')],
        string="Current Running Server DB", default="local_server")
    remote_server_url = fields.Char(string="Remote Server Url")
    remote_server_db = fields.Char(string="Remote Server DB")
    remote_server_admin_user_name = fields.Char(string="Remote Server Admin User Name")
    remote_server_admin_user_password = fields.Char(string="Remote Server Admin User Passwords")
    remote_server_last_synced_data_time = fields.Datetime(string="Remote Server Data Last Synced Date Time")
    ...
    def is_server_accessible(self, url):
        try:
            response = requests.get(url, timeout=100)  # Timeout after 5 seconds
    ...
    def get_model_fields(self, model):
        ...
        fields_list = [field_name for field_name in self.env[model]._model_fields if field_name not in pop_list]
        return fields_list
    ...
    def process_many2one_field(self, comodel_name, field_value, remote_server_db, uid,
        ...
        if 'active' in list(self.env[comodel_name]._fields):
            related_records_domain.append(('active','in',[True,False]))
        related_record = self.env[comodel_name].search(related_records_domain,limit=1)
        if related_record:
            ...
            return self.env[comodel_name].create(related_data[0]).id
            ...
            if related_record:
                ...
                for field, value in related_data[0].items():
                    related_data[0][field] = self.handle_field_type(comodel_name, field, value,
                                                                    ...
                                                                    remote_db_access_models)
                related_data[0].pop('id')
                created_record = self.env[comodel_name].create(related_data[0])
            ...
            if related_record:
                ...
                for field, value in related_data[0].items():
                    related_data[0][field] = self.handle_field_type(comodel_name, field, value,
                                                                    ...
                                                                    remote_db_access_models)
                related_data[0].pop('id')
                new_record = self.env[comodel_name].create(related_data[0])
    ...
    def handle_field_type(self, model_name, field_name, field_value, remote_server_data, remote_server_db, uid,
        ...
        fields_comodel_relation = self.env['ir.model.fields'].search([
    ...
    def process_record(self, model, server_rec, remote_server_db, uid, remote_server_admin_user_password,
        ...
        try:
            ...
            rec_in_local_db = self.env[model].search([('remote_server_db_id', '=', server_rec_db_id)])
            if rec_in_local_db:
                rec_in_local_db.sudo().write(server_rec)  # Update the record
            else:
                self.env[model].sudo().create(server_rec)  # Create new record
        except Exception as e:
            print(e)
        _logger.info(f"Processed {model} record with remote_server_db_id {server_rec_db_id}")
    # @http.route()
    def fetch_remote_updateds_data_cron(self, config_id=False, **k):
        if self.env.company.current_running_db_server == 'local_server':
            last_remote_data_fetched_date_utc = self.env.company.remote_server_last_synced_data_time
            ...
            if current_running_db_server == 'local_server' and remote_server_url and self.is_internet_available() and self.is_server_accessible(
                ...
                for model in models_list:
                    ...
                    if model in ['product.template']:
                        updates_domain = [('syncable_fields_last_update_date_time', '>', last_remote_data_fetched_date_utc)]
                    if self.env[model]._fields.get('active'):
                        updates_domain.append(('active', '=', True))
                    if self.env[model]._fields.get('company_id') and model not in ['product.template',
                                                                                      'product.pricelist',
                                                                                      'product.pricelist.item',
                                                                                      'res.partner']:
                        updates_domain.append(('company_id', '=', self.env.company.remote_server_db_id))
                ...
                self.env.company.remote_server_last_synced_data_time = datetime.now()
