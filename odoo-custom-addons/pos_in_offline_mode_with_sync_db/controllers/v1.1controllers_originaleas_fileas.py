...
class PosControllerPosOffline(PosController):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        # def send_latest_data_updates_to_the_server(self, recently_sendings_date):
        if request.env.company.current_running_db_server == 'local_server':
            ...
            if current_running_db_server == 'local_server' and remote_server_url and self.is_internet_available() and self.is_server_accessible(
                ...
                # , 'product.removal', 'ir.sequence''pos.payment.method','product.attribute', 'product.attribute.value', 'product.template.attribute.value'
                # 'res.users' after the res.partnerereas
                models_list = ['account.tax.group', 'account.tax', 'product.category',

                               'pos.category', 'stock.location', 'stock.warehouse', 'stock.picking.type',
                               ...
                               'account.payment.term', 'account.payment.term.line', 'crm.team',
                               'res.partner.category', 'res.bank', 'res.partner.bank', 'product.pricelist',
                ...
                needs_to_update = []
                # Needss toeas aneas creatingyieas
                needs_to_create = []
                for model in models_list:
                    # current_model_available_remote_db_ids = request.env[model].search(
                    #     [('remote_server_db_id', 'not in', [False, 0])]).mapped('remote_server_db_id')

                    # , ('id', 'in', current_model_available_remote_db_ids)
                    ...
                    needs_to_update = models.execute_kw(remote_server_db, uid, remote_server_admin_user_password,
                    ...
                    # field_type = field.type
                    # parent_id =
                    same_comodel_rec_fields = request.env['ir.model.fields'].search([
                        ('model_id', '=', model_rec.id),
                        ('name', 'in', FIELDS[model])
                    ]).filtered(lambda f: f.relation == model)
                    # fields_comodel_relation =
                    originals_needs_to_update = copy.deepcopy(needs_to_update)
                    for server_rec in needs_to_update:
                        ...
                        for field_name, field_value in server_rec.items():
                            # and field_name != same_comodel_rec_fields.name
                            # if model != same_comodel_rec_fields.relation and field_name != same_comodel_rec_fields.name:
                            server_rec[field_name] = self.handle_field_type(model, field_name, field_value,
                                                                            server_rec, remote_server_db, uid,
                                                                            remote_server_admin_user_password,
                                                                            remote_db_access_models)
                            # else:
                            # Needs to removings ans same comdels fields datas n to updates, it  iwlls ns upfationga sn afters all recodeas process ands  creatingea saorss ans updatingyieas
                        if same_comodel_rec_fields:
                            for same_fields in same_comodel_rec_fields:
                                server_rec.pop(same_fields.name)
                        rec_in_local_db = request.env[model].search([('remote_server_db_id', '=', server_rec_db_id)])
                        if rec_in_local_db:
                            rec_in_local_db.sudo().write(server_rec)
                        else:
                            request.env[model].sudo().create(server_rec)
                    # if needs_to_update:
                    #     # remote_server_db_ids = map()
                    #     request.env[model].write(needs_to_update)

                    # Needs to removings ans same comdels fields datas n to updates, it  iwlls ns upfationga sn afters all recodeas process ands  creatingea saorss ans updatingyieas
                    if same_comodel_rec_fields and model == same_comodel_rec_fields[0].relation:
                        for server_rec in originals_needs_to_update:
                            for same_fields in same_comodel_rec_fields:
                                comodel_same_fields_name = same_fields.name
                                server_rec_db_id = server_rec.get('id')
                                rec_in_local_db = request.env[model].search(
                                    [('remote_server_db_id', '=', server_rec_db_id)])

                                # try:
                                server_comodel_related_fields_rec_id = False
                                same_fields_type = request.env[model]._fields[same_fields.name].type
                                if server_rec[same_fields.name] and same_fields_type == 'many2one':
                                    server_comodel_related_fields_rec_id = server_rec[same_fields.name][0]
                                elif server_rec[same_fields.name] and same_fields_type in ['one2many', 'many2many']:
                                    server_comodel_related_fields_rec_id = server_rec[same_fields.name]
                                # False
                                # except Exception as e:
                                print("Hiieas")
                                if server_comodel_related_fields_rec_id and same_fields_type == 'many2one':
                                    related_comodel_fields_local_rec_id = request.env[model].search(
                                        [('remote_server_db_id', '=', server_comodel_related_fields_rec_id)])
                                    rec_in_local_db.write(
                                        {comodel_same_fields_name: related_comodel_fields_local_rec_id.id})
                                elif server_comodel_related_fields_rec_id and same_fields_type in ['one2many',
                                                                                                   'many2many']:
                                    related_comodel_fields_local_rec_id = request.env[model].search(
                                        [('remote_server_db_id', 'in', server_comodel_related_fields_rec_id)])
                                    rec_in_local_db.write(
                                        {comodel_same_fields_name: related_comodel_fields_local_rec_id.ids})
                    _logger.info(f"{model} =>Records Updates ins locals db")

                    # ('active', '=', True),
                    # create_domain = [('id', 'not in', current_model_available_remote_db_ids),
                    #                  ('create_date', '>', last_remote_data_fetched_date_utc)]
                    # needs_to_create = models.execute_kw(remote_server_db, uid, remote_server_admin_user_password,
                    #                                     model, 'search_read', [create_domain],
                    #                                     {'fields': FIELDS[model]})
                    # for server_rec in needs_to_create:
                    #     server_rec_db_id = server_rec.get('id')
                    #     server_rec.pop('id')
                    #     server_rec.update({'remote_server_db_id': server_rec_db_id})
                    # if needs_to_create:
                    #     request.env[model].create(needs_to_create)
                    #     _logger.info(f"{model} => New Records successfully fetched ins locals db")

                # request.env["ir.config_parameter"].sudo().set_param(
                #     "pos_in_offline_mode_with_sync_db.last_remote_data_fetched_date_utc", datetime.utcnow())
