...
class PrintProductLabel(models.TransientModel):
    ...
    def _get_products(self):
        ...
        if self._context.get('active_model') == 'product.template':
            ...
            for product in products:
                ...
                res.append(label.id)
        elif self._context.get('wizard_active_model') == 'product.product':
            products = self.env[self._context.get('wizard_active_model')].browse(
                self._context.get('default_product_ids'))
            for product in products:
                label = self.env['print.product.label.line'].create({
                    'product_id': product.id,
    ...
    border_width = fields.Integer(
        string='Border',
        help='Border width for labels (in pixels). Set "1" for no border.'
    )
