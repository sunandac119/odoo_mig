...
class ResCompany(models.Model):
    ...
    sh_display_vendor_statement = fields.Boolean('Show Vendor Statement Menu in portal ?')
...
class ResConfigSetting(models.TransientModel):
    ...
    sh_vendor_statement_auto_send = fields.Boolean(
        'Vendor Statement Auto Send', related='company_id.sh_vendor_statement_auto_send', readonly=False)
    filter_only_unpaid_bill_and_send_that = fields.Boolean(string = "Filter Only Unpaid, Send nothing if all bills are paid",
    related='company_id.filter_only_unpaid_bill_and_send_that',readonly=False)
    
    sh_vendor_statement_action = fields.Selection(
        related='company_id.sh_vendor_statement_action', string='Vendor Statement Action', readonly=False)
    sh_vend_daily_statement_template_id = fields.Many2one(
        'mail.template', string='Daily Mail Template', related='company_id.sh_vend_daily_statement_template_id', readonly=False)
    sh_vend_week_day = fields.Selection(
        string='Week Day', related='company_id.sh_vend_week_day', readonly=False)
    sh_vend_weekly_statement_template_id = fields.Many2one(
        'mail.template', string='Weekly Mail Template', related='company_id.sh_vend_weekly_statement_template_id', readonly=False)
    sh_vend_monthly_date = fields.Integer(
        'Monthly Day', related='company_id.sh_vend_monthly_date', readonly=False, default=1)
    sh_vend_monthly_template_id = fields.Many2one(
        'mail.template', string='Monthly Mail Template', related='company_id.sh_vend_monthly_template_id', readonly=False)
    sh_vend_yearly_date = fields.Integer(
        'Yearly Day', related='company_id.sh_vend_yearly_date', readonly=False, default=1)
    sh_vend_yearly_month = fields.Selection(
        string='Month', related='company_id.sh_vend_yearly_month', readonly=False)
    sh_vend_monthly_end = fields.Boolean(
        'End of month', related='company_id.sh_vend_monthly_end', readonly=False)
    sh_vend_yearly_template_id = fields.Many2one(
        'mail.template', string='Yearly Mail Template', related='company_id.sh_vend_yearly_template_id', readonly=False)
    sh_vend_create_log_history = fields.Boolean(
        'Vendor Statement Mail Log History', related='company_id.sh_vend_create_log_history', readonly=False)

    sh_vendor_due_statement_auto_send = fields.Boolean(
        'Vendor Overdue Statement Auto Send', related='company_id.sh_vendor_due_statement_auto_send', readonly=False)
    sh_vendor_due_statement_action = fields.Selection(
        related='company_id.sh_vendor_due_statement_action', string='Vendor Overdue Statement Action', readonly=False)
    sh_vend_due_daily_statement_template_id = fields.Many2one(
        'mail.template', string='Daily Mail Template', related='company_id.sh_vend_due_daily_statement_template_id', readonly=False)
    sh_vend_due_week_day = fields.Selection(
        string='Week Day', related='company_id.sh_vend_due_week_day', readonly=False)
    sh_vend_due_weekly_statement_template_id = fields.Many2one(
        'mail.template', string='Weekly Mail Template', related='company_id.sh_vend_due_weekly_statement_template_id', readonly=False)
    sh_vend_due_monthly_date = fields.Integer(
        'Monthly Day', related='company_id.sh_vend_due_monthly_date', readonly=False, default=1)
    sh_vend_due_monthly_end = fields.Boolean(
        'End of month', related='company_id.sh_vend_due_monthly_end', readonly=False)
    sh_vend_due_monthly_template_id = fields.Many2one(
        'mail.template', string='Monthly Mail Template', related='company_id.sh_vend_due_monthly_template_id', readonly=False)
    sh_vend_due_yearly_date = fields.Integer(
        'Yearly Day', related='company_id.sh_vend_due_yearly_date', readonly=False, default=1)
    sh_vend_due_yearly_month = fields.Selection(
        string='Month', related='company_id.sh_vend_due_yearly_month', readonly=False)
    sh_vend_due_yearly_template_id = fields.Many2one(
        'mail.template', string='Yearly Mail Template', related='company_id.sh_vend_due_yearly_template_id', readonly=False)
    sh_vend_due_create_log_history = fields.Boolean(
        'Vendor Overdue Statement Mail Log History', related='company_id.sh_vend_due_create_log_history', readonly=False)
    sh_display_customer_statement = fields.Boolean('Show Customer Statement Menu in portal ?',
        readonly=False,
    ...
    sh_display_vendor_statement = fields.Boolean('Show Vendor Statement Menu in portal ?',
        readonly=False,
        related='company_id.sh_display_vendor_statement'
    )
